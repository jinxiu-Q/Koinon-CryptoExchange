...
//        }
    ...
    public Page<ExchangeOrder> personalCurrentOrder(@SessionAttribute(SESSION_MEMBER) AuthMember member,
        ...
        page.getContent().forEach(exchangeOrder -> {
            ...
            if (exchangeOrder.getBaseSymbol().equalsIgnoreCase("USDT")) {
                exchangeOrder.setUsdtValue(exchangeOrder.getPrice().multiply(exchangeOrder.getAmount()));
            } else {
                exchangeOrder.setUsdtValue(exchangeOrder.getPrice().multiply(exchangeOrder.getAmount()).multiply(getUsdRate(exchangeOrder.getBaseSymbol())));
            }
        });
        return page;
    }
    ...
    private BigDecimal getUsdRate(String unit) {
        String url = "http://bitrade-market/market/exchange-rate/usd/{coin}";
...
//        });
    ...
    public List<ExchangeOrderDetail> currentOrder2(@SessionAttribute(SESSION_MEMBER) AuthMember member, String orderId) {
        return exchangeOrderDetailService.findAllByOrderId(orderId);
    }

    @RequestMapping("base")
    public MessageResult base(@SessionAttribute(SESSION_MEMBER) AuthMember member, String orderId) {
        ExchangeOrder order = orderService.findOne(orderId);
        MessageResult r = MessageResult.success();
        r.setData(order);
        return r;
    }

    @RequestMapping("cancel")
    public MessageResult cancelOrder2(@SessionAttribute(SESSION_MEMBER) AuthMember member, String orderId) {
        ExchangeOrder order = orderService.findOne(orderId);
