...
//    private ESUtils esUtils;
    ...
    public void handle(String content) {
        ...
        for (Coin coin : coins) {
            ...
            if (coin.getEnableRpc() == BooleanEnum.IS_TRUE) {
                ...
                try {
                    ...
                    if (result.getStatusCode().value() == 200) {
                        ...
                        logger.info("mr={}", mr);
                        if (mr.getCode() == 0) {
                            String address = (String) mr.getData();//返回地址成功，调用持久化
            ...
            if (wallet.getAddress().length() > 0) {
                if ("ETH".equals(coin.getUnit()) || "BTC".equals(coin.getUnit()) || "TRX".equals(coin.getUnit())) {
                    Addressext addressext = new Addressext();
                    addressext.setMemberId(wallet.getMemberId().intValue());
                    addressext.setAddress(wallet.getAddress());
                    if ("ETH".equals(coin.getUnit()))
                        addressext.setCoinProtocol(1);
                    else if ("BTC".equals(coin.getUnit()))
                        addressext.setCoinProtocol(2);
                    else if ("TRX".equals(coin.getUnit()))
                        addressext.setCoinProtocol(3);
