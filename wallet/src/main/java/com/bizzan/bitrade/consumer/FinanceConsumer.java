...
public class FinanceConsumer {
    ...
    public void handleDeposit(ConsumerRecord<String, String> record) {
        ...
        String fromAddress = json.getString("srcAddress");
        ...
        if (coin != null && walletService.findDeposit(address, txid) == null) {
            MessageResult mr = walletService.recharge(coin, address, amount, txid, fromAddress, symbol, blockHeight);
...
//        }
    ...
    private String getRpcName(Long protocol) {
        if (protocol == 1) {
            return "ETH";
        } else if (protocol == 2) {
            return "BTC";
    ...
    public void handleWithdraw(ConsumerRecord<String, String> record) {
        ...
        JSONObject json = JSON.parseObject(record.value());
        Long protocol = json.getLong("protocol");
        Long withdrawId = json.getLong("withdrawId");
        String unit = record.key().toUpperCase();
        try {
            String serviceName = "SERVICE-RPC-" + getRpcName(protocol);
            ///rpc/withdraw/unit?address={1}&amount={2}&fee={3}
            String url = "http://" + serviceName + "/rpc/withdraw/" + unit + "?address={1}&amount={2}&fee={3}";
            Coin coin = coinService.findByUnit(record.key());
            ...
            if (coin != null && coin.getCanAutoWithdraw() == BooleanEnum.IS_TRUE) {
                ...
                if (result.getCode() == 0 && result.getData() != null) {
                    logger.info("====================== 处理成功,data为txid,更新业务 ==================================");
