...
//    private double gcxMatchEachLimit;
    ...
    @RequestMapping("transaction/dw")
    public MessageResult findTransactionDW(@SessionAttribute(SESSION_MEMBER) AuthMember member, int pageNo, int pageSize) {
        MessageResult mr = new MessageResult();
        mr.setData(transactionService.queryByMember(member.getId(), pageNo, pageSize));
        mr.setCode(0);
        mr.setMessage("success");
        return mr;
    }
