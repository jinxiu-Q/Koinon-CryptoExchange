...
//    private String emailTag;
    ...
    @RequestMapping(value = "/support/country/app", method = RequestMethod.POST)
    @ResponseBody
    public MessageResult allAppCountry() {
        MessageResult result = success();
        List<Country> list = countryService.getAllCountryApp();
        result.setData(list);
        return result;
    }
    ...
    @RequestMapping(value = "/register/check/email")
    @ResponseBody
    public MessageResult checkEmail(String email) {
        MessageResult result = success();
...
//        isTrue(memberService.userPromotionCodeIsExist(loginByPhone.getPromotion()),localeMessageSourceService.getMessage("USER_PROMOTION_CODE_EXISTS"));
    ...
    public MessageResult sendBindEmail(String email, @SessionAttribute(SESSION_MEMBER) AuthMember user) {
        ...
        try {
            sentEmailAddCode("Verification Code", email, code, "bindCodeEmail.ftl");
...
//    }
    ...
    public MessageResult sendAddAddress(@SessionAttribute(SESSION_MEMBER) AuthMember user) {
        ...
        try {
            sentEmailAddCode("Verification Code", email, code, "addAddressCodeEmail.ftl");
    ...
    @Async
    public void sentEmailAddCode(String subject, String email, String code, String flt) throws MessagingException, IOException, TemplateException {
        MimeMessage mimeMessage = javaMailSender.createMimeMessage();
        MimeMessageHelper helper = null;
        helper = new MimeMessageHelper(mimeMessage, true);
    ...
    public MessageResult sendResetPasswordCode(String account) {
        ...
        try {
            String code = String.valueOf(GeneratorUtil.getRandomNumber(100000, 999999));
            sentEmailAddCode("Verification Code", account, code, "resetPasswordCodeEmail.ftl");
...
//    }
    ...
    public MessageResult untieEmailCode(@SessionAttribute(SESSION_MEMBER) AuthMember user) {
        ...
        String code = String.valueOf(GeneratorUtil.getRandomNumber(100000, 999999));
        try {
            sentEmailAddCode("Verification Code", member.getEmail(), code, "resetPasswordCodeEmail.ftl");
    ...
    public MessageResult updateEmailCode(@SessionAttribute(SESSION_MEMBER) AuthMember user, String email) {
        ...
        String code = String.valueOf(GeneratorUtil.getRandomNumber(100000, 999999));
        try {
            sentEmailAddCode("Verification Code", email, code, "resetPasswordCodeEmail.ftl");
            valueOperations.set(RESET_PASSWORD_CODE_PREFIX + email, code, 10, TimeUnit.MINUTES);
...
//    }
     ...
     * @throws Exception
     */
    @RequestMapping("/register/for_app")
    @ResponseBody
//    @Transactional(rollbackFor = Exception.class)
    public MessageResult registerApp(
            @Valid RegisterByApp registerByApp,
            BindingResult bindingResult, HttpServletRequest request) throws Exception {
        MessageResult result = BindingResultUtil.validate(bindingResult);
        if (result != null) {
            log.info("registerApp for {}", registerByApp.toString());
            return result;
        }
        Country country = countryService.findOne(registerByApp.getCountry());
        if (country == null) {
            country = countryService.findOneE(registerByApp.getCountry());
        }
        String ip = request.getHeader("X-Real-IP");
        isTrue(!memberService.emailIsExist(registerByApp.getEmail()), localeMessageSourceService.getMessage("EMAIL_ALREADY_BOUND"));
        isTrue(!memberService.usernameIsExist(registerByApp.getUsername()), localeMessageSourceService.getMessage("USERNAME_ALREADY_EXISTS"));
        ////生成密码,不可重复随机数,盐
        String loginNo = String.valueOf(idWorkByTwitter.nextId());
        String credentialsSalt = ByteSource.Util.bytes(loginNo).toHex();
        String password = Md5.md5Digest(registerByApp.getPassword() + credentialsSalt).toLowerCase();
        Member member = new Member();
        //需要2小时内激活
        member.setStatus(CommonStatus.ILLEGAL);
        member.setMemberLevel(MemberLevelEnum.GENERAL);
        Location location = new Location();
        location.setCountry(country.getZhName());

        Country countryTemp = new Country();
        countryTemp.setZhName(country.getZhName());
        member.setCountry(countryTemp);
        member.setLocation(location);
        member.setUsername(registerByApp.getUsername());
        member.setPassword(password);
        member.setEmail(registerByApp.getEmail());
        member.setSalt(credentialsSalt);
        Member member1 = memberService.save(member);
        if (member1 != null) {
            //发送邮件激活码
            sendRegEmailForApp(registerByApp.getEmail());
            return success(localeMessageSourceService.getMessage("REGISTRATION_SUCCESS"));
        } else {
            return error(localeMessageSourceService.getMessage("REGISTRATION_FAILED"));
        }
    ...
    public MessageResult sendRegEmailForApp(String email) {
        Assert.isTrue(ValidateUtil.isEmail(email), localeMessageSourceService.getMessage("WRONG_EMAIL"));
        String code = String.valueOf(GeneratorUtil.getRandomNumber(100000, 999999));
        ValueOperations valueOperations = redisTemplate.opsForValue();
//        if (valueOperations.get(EMAIL_REG_CODE_PREFIX + email) != null) {
...
//        }
        try {
            sentEmailAddCode("Welcome to Kraken", email, code, "welcome.ftl");
            log.info("sendRegEmailForApp code" + code);
            valueOperations.set(EMAIL_REG_CODE_PREFIX + email, code, 120, TimeUnit.MINUTES);
        } catch (Exception e) {
            e.printStackTrace();
            log.error(e.getMessage());
            return error(localeMessageSourceService.getMessage("SEND_FAILED"));
    ...
    @RequestMapping("/register/for_app_active")
    @ResponseBody
    public MessageResult registerByEmailActive(@Valid RegisterByAppActive appactive, BindingResult bindingResult) throws Exception {
        MessageResult result = BindingResultUtil.validate(bindingResult);
        if (result != null) {
            return result;
        }
        isTrue(memberService.usernameIsExist(appactive.getUsername()), localeMessageSourceService.getMessage("ACTIVATION_FAILS"));
        Member member = memberService.findByUsername(appactive.getUsername());
        String email = member.getEmail();
        ValueOperations valueOperations = redisTemplate.opsForValue();
        Object code = valueOperations.get(SysConstant.EMAIL_REG_CODE_PREFIX + email);
        notNull(code, localeMessageSourceService.getMessage("VERIFICATION_CODE_NOT_EXISTS"));
        if (!code.toString().equals(appactive.getCode())) {
            return error(localeMessageSourceService.getMessage("VERIFICATION_CODE_INCORRECT"));
        } else {
            valueOperations.getOperations().delete(SysConstant.EMAIL_REG_CODE_PREFIX + email);
        }
